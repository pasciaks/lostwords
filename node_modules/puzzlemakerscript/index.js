...
var PuzzleMakerScript = {
    ...
    addWord: function(word, howManyTimes, dx, dy, howManyAttempts) {
        ...
        this.allWords.push(theWordsAdded);
     ...
     * 
     * */
    createPath: function(sx, sy, word, allowReuseLetters, repickDirections, dirchangepercent) {
        
        if (typeof repickDirections=='undefined') repickDirections = false;
        
        if (typeof dirchangepercent=='undefined') dirchangepercent = 25;
        
        var usedLetters={}; // usedLetters['sx-sy']
        ...
        var thePath = []; // [ { sx, sy, letter }, { sx, sy, letter } ]  ???
        
        var toomany=0;

        if (this.get(sx, sy) !== this.emptyChar) {
            if (allowReuseLetters==false) {
                //return { 'result':'bad start', 'thePath': [] };
                ...
                if (++toomany>10000) return { 'result':'bad start', 'thePath': [] };
                    
                } while ( (this.get(sx, sy) !== this.emptyChar) || ( (allowReuseLetters==true) && (this.get(sx,sy)!==word[0])  ) );
            }
        }
        ...
        var savedPuzzle = this.thePuzzle;
        ...
        for ( var i=0;i<word.length;i++ ) {
            ...
            if (this.get(sx,sy)!==this.emptyChar) {
                ...
                if ((this.get(sx,sy)==word[i]) && (allowReuseLetters==true)) {
                    ...
                    //console.log("***************************************************");
                    this.thePuzzle=savedPuzzle;
                    return { 'result':'already used letter and no reuse allowed', 'thePath': [] };
                }
            ...
            //this.thePuzzle=this.thePuzzle;
            
            usedLetters[sx+'-'+sy]=word[i];
            
            limit=10000;
            
            if (++failSafe>10000) {console.log("faile safe");return { 'result':'fail safe', 'thePath': [] };} //break;
            ...
            do {
                ...
                if (repickDirections) {
                    if (Math.random()*100>dirchangepercent) {
                        dir=this.pickDirection();
                    }
                }
                ...
                if (sx<0) failed=true;
                if (sx>=this.cols) failed=true;
                if (sy<0) failed=true;
                if (sy>=this.rows) failed=true;
                
                if (failed) {
                    limit=0;
                } else {
                    if ( ( this.get(sx,sy)==this.emptyChar ) || ((allowReuseLetters) && (this.get(sx,sy)==word[i])) ){
                        //this.set(sx,sy,word[i]);
                        limit = -100;
                        break;
                    }
            ...
            if (limit<0) if (limit>-50) { this.thePuzzle=savedPuzzle; return { 'result':'out of bounds', 'thePath': [] }; }
